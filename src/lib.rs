...
use notify::{recommended_watcher, RecommendedWatcher, RecursiveMode, Watcher, Result as NotifyResult, Event};
...
#[pymodule]
mod _watch {
    use notify::{event::{CreateKind, ModifyKind, RenameMode}, EventKind};
    ...
    impl Watch {
        ...
        fn new<'py>() -> PyResult<Self> {
            ...
            Ok(Self {
                watcher: Mutex::new(recommended_watcher(tx).map_err(|e| PyOSError::new_err(format!("failed to watch: {e}")))?),
                rx: Mutex::new(RefCell::new(Some(rx)))
            })
        }
        
        fn add_watch(&self, to_watch: PathBuf) -> PyResult<()> {
            self.watcher.try_lock()
        ...
        fn __anext__<'py>(slf: Bound<'py, Self>, _py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {    
            ...
            let rx_cell = match slf.get().rx.try_lock() {
               Ok(lock) => lock,
               Err(e) => return Err(PyRuntimeError::new_err(format!("the read lock is being held by someone else (mutex error: {e})")))
            };
            
            let Some(rx) = rx_cell.take() else {
                return Err(PyRuntimeError::new_err("the read lock is being held by something else (the lock was None)"))
            };
            
            match rx.try_recv() {
                Ok(ev) => {
                    let event = ev
                        .map_err(|e| PyOSError::new_err(format!("watch error: {e}")))?;
